...
async function main() {
    try {
        ...
        // Fix Windows path escaping issues in JSON
        // Replace single backslashes with double backslashes for JSON parsing
        ...
        if (!projectDir) {
            // Fallback: go up two levels from .claude/hooks to project root
            const cwd = process.cwd();
            projectDir = cwd.replace(/\.claude[\/\\]hooks$/, '');
        }
        const rulesPath = join(projectDir, '.claude', 'skills', 'skill-rules.json');

        // Load skill rules with better error handling
        ...
        try {
            ...
            console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
            // Exit with code 1 to show error in verbose mode (Ctrl+O)
            process.exit(1);
        }
